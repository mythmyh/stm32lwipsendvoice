...
//	i2srecbuf1 = (uint8_t*)malloc(4096);
...
//f_mkdir("0:\\etcdh");
	...
	WM8978_I2S_Cfg(2, 0);//飞利浦标�??????????????????????????,16位数据长�??????????????????????????
	printf("===>%d\n",AKA);
...
//	uint8_t wr_data[] = "Welcome to EmbeTronicX!!!";
//	FIL file;
//	res2 = f_open(&file, "hello.txt", (FA_CREATE_ALWAYS | FA_WRITE));
//	printf("f_open %d %d\n", res2,FR_OK);
//
